"""books_from_ws（原 books_from_feed）新鲜度门控回归测试。

复现并验证根因修复：``LocalOrderBook.ready`` 是单向锁（收到首个快照即永久
True），仅凭 ready 判定会让策略的 REST 回退分支成为死代码——WS 静默
（连接未断但服务端不再推 book/price_change）时，本地簿冻结在旧值，策略却
毫不知情地继续用它下单。叠加 ``is_fresh`` 阈值后，陈旧盘口会被识别，
``books_from_ws`` 返回 None，调用方据此回退 REST 快照。

阶段 1 commit A：该逻辑由 app/trade5m.py 搬至 strategies/crypto_5m/context.py
（WindowDataHub），本测试同步迁移。
"""
...
from pm_arb.strategies.crypto_5m.context import WindowDataHub
...
def _hub(feed: MarketDataFeed) -> WindowDataHub:
    """构造只依赖本地簿的 hub（rest/rtds 不被 books_from_ws 触及，可传 None）。"""
    return WindowDataHub(feed, None, None, UP, DOWN, ws_fresh_sec=P.ws_fresh_sec)
...
def _make_stale(feed: MarketDataFeed, asset_id: str) -> None:
    ...
    feed.book(asset_id)._last_update = time.monotonic() - (P.ws_fresh_sec + 1000)


def test_returns_books_when_both_fresh():
    feed = _feed_with_fresh_books()
    b = _hub(feed).books_from_ws()
...
def test_returns_none_when_stale_even_though_ready():
    ...
    assert _hub(feed).books_from_ws() is None
